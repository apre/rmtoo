...
class csv1(StdOutputParams, ExecutorTopicContinuum,
    ...
    def requirement(self, req):
        ...
        priority = req.get_requirement().get_value("Priority")
        effort = "n/a"
        if req.get_requirement().is_val_av_and_not_null("Effort estimation"):
            effort = req.get_requirement().get_value("Effort estimation")
        ...
        testcases = ""
        if req.get_requirement().is_val_av_and_not_null("Test Cases"):
            testcases = req.get_requirement().get_value("Test Cases")
        ...
        self.__fd.write('requirement,"%s",%4.3f,"%s","%s","%s","%s","%s"\n' 
                        % (status, priority, effort, req.get_requirement().get_id(), desc,
